...
{
    private readonly record struct ImagePair(MaterialDefinition Tile, Image Image);
    ...
    private MaterialDefinition[] _resultTiles;
    ...
    public MaterialLoader(
        string[] folders, 
        Func<string, Serilog.ILogger?, Task<MaterialDefinition[]>> asyncInitParser,
    ...
    )
    {
        ...
        _loadTask = Task.Factory.StartNew(() => {
            List<(string folder, Task<MaterialDefinition[]> task)> imported = [];
            var tileCount = 0;
            ...
            foreach (var t in filtered)
            {
                var imagePairTasks = t.task.Result
                    .Where(tile => {
                        var exists = Program.FileExistsInFolder(t.folder, $"{tile.Name}Texture.png");
                    ...
                    .Select(tile => {
                        string imagePath = Program.GetFilePathInFolder(t.folder, $"{tile.Name}Texture.png")!;
            ...
            _resultTiles = new MaterialDefinition[tileCount];
    ...
    /// </summary>
    /// <returns></returns>
    public MaterialDefinition[] GetResult() => _resultTiles;
}