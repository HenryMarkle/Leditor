...
public class MaterialDex
{
    ...
    /// Tile -> Definition
    /// </summary>
    private readonly ImmutableDictionary<string, Material> _definitions;
    
    /// <summary>
    /// Category -> Definition[]
    /// </summary>
    private readonly ImmutableDictionary<string, Material[]> _categories;
    
    /// <summary>
    /// Tile -> Category
    /// </summary>
    private readonly ImmutableDictionary<Material, string> _materialCategory;
    ...
    public Material GetMaterial(string name) => _definitions[name];
    ...
    public bool TryGetMaterial(string name, out Material? definition) =>
    ...
    public Material[] GetMaterialsOfCategory(string name) => _categories[name];
    ...
    public bool TryGetMaterialsOfCategory(string name, out Material[]? materials) => _categories.TryGetValue(name, out materials);
    ...
    public string GetCategory(Material material) => _materialCategory[material];
    ...
    /// <returns>true if found; otherwise false</returns>
    /// <exception cref="KeyNotFoundException">The tile name is not found</exception>
    public bool TryGetCategory(Material tile, out string? category) => _materialCategory.TryGetValue(tile, out category);
    ...
    /// <returns>A color struct</returns>
    /// <exception cref="KeyNotFoundException">The tile name is not found</exception>
    public Color GetTileColor(Material tile) => _materialColor[tile.Name];
    ...
    /// <param name="color">The found color</param>
    /// <returns>true if found; otherwise false</returns>
    public bool TryGetTileColor(Material tile, out Color color) => _materialColor.TryGetValue(tile.Name, out color);
    ...
    public MaterialDex(
        Dictionary<string, Material> definitions,
        Dictionary<string, Material[]> categories,
        Dictionary<Material, string> materialCategory,
