...
internal class ExperimentalGeometryPage : EditorPage
{
    ...
    private bool _mirrorCursor = true;
    private bool _fourthMirrorCursor;
    private bool _labeledMirrorCursors;

    private int _verticalMirrorPos = -1;
    private int _horizontalMirrorPos = -1;

    private int _movingMirrorLock;
    ...
    private bool _isMirrorWinHovered;
    ...
    public override void Draw()
    {
        ...
        if (_verticalMirrorPos == -1) _verticalMirrorPos = GLOBALS.Level.Width * 20 / 2;
        if (_horizontalMirrorPos == -1) _horizontalMirrorPos = GLOBALS.Level.Height * 20 / 2;

        var _verticalMirrorHover = _verticalMirror && mouse.X >= _verticalMirrorPos - 5f && mouse.X <= _verticalMirrorPos + 5f;
        var _horizontalMirrorHover = _horizontalMirror && mouse.Y >= _horizontalMirrorPos - 5f && mouse.Y <= _horizontalMirrorPos + 5f;

        if (_verticalMirror || _horizontalMirror)
        {
            if (_verticalMirrorHover && _horizontalMirrorHover)
            {
                SetMouseCursor(MouseCursor.ResizeAll);
            }
            else if (_verticalMirrorHover)
            {
                SetMouseCursor(MouseCursor.ResizeEw);
            }
            else if (_horizontalMirrorHover)
            {
        ...
        var canDrawGeo = !_isNavbarHovered &&
                         ...
                         !(_verticalMirrorHover || _horizontalMirrorHover || _movingMirrorLock != 0) &&
        ...
        // Handle moving mirros

        if (!_clickTracker && (_verticalMirror || _horizontalMirror) && (_horizontalMirrorHover || _verticalMirrorHover))
        {
            if (_movingMirrorLock == 0 && IsMouseButtonDown(MouseButton.Left))
            {
                if (_horizontalMirrorHover && _verticalMirrorHover)
                {
                    _movingMirrorLock = 3;
                }
                else if (_horizontalMirrorHover)
        ...
        if (_movingMirrorLock != 0)
        {
            ...
            switch (_movingMirrorLock)
            {
                case 1:
                    _verticalMirrorPos = matrixX * 20;
                    break;

                case 2:
                    _horizontalMirrorPos = matrixY * 20;
                    break;
        ...
        if (_shortcuts.ToggleVerticalMirror.Check(ctrl, shift, alt))
        {
            _verticalMirror = !_verticalMirror;
        }
        if (_shortcuts.ToggleHorizontalMirror.Check(ctrl, shift, alt))
        {
            _horizontalMirror = !_horizontalMirror;
        }

        if (_shortcuts.MoveVerticalMirrorRight.Check(ctrl, shift, alt))
        {
            _verticalMirrorPos += 20;
        }
        else if (_shortcuts.MoveVerticalMirrorLeft.Check(ctrl, shift, alt))
        {
            _verticalMirrorPos -= 20;
        }

        if (_shortcuts.MoveHorizontalMirrorUp.Check(ctrl, shift, alt))
        {
            _horizontalMirrorPos -= 20;
        }
        else if (_shortcuts.MoveHorizontalMirrorDown.Check(ctrl, shift, alt))
        {
            _horizontalMirrorPos += 20;
        }
        ...
        if (_allowMultiSelect)
        {
            ...
            if ((IsMouseButtonReleased(_shortcuts.Draw.Button) || IsKeyReleased(_shortcuts.AltDraw.Key)) && _prevCoordsX != -1)
            {
                ...
                else
                {
                    ...
                    // Vertical Mirror
                    var newVMCopy = _verticalMirror ? new Geo[endY - startY + 1, endX - startX + 1] : new Geo[0, 0];
                    var oldVMCopy = _verticalMirror ? new Geo[endY - startY + 1, endX - startX + 1] : new Geo[0, 0];

                    // Horizontal Mirror
                    var newHMCopy = _horizontalMirror ? new Geo[endY - startY + 1, endX - startX + 1] : new Geo[0, 0];
                    var oldHMCopy = _horizontalMirror ? new Geo[endY - startY + 1, endX - startX + 1] : new Geo[0, 0];
                    
                    // Fourth Mirror
                    var newXMCopy = _verticalMirror && _horizontalMirror && _fourthMirrorCursor ? new Geo[endY - startY + 1, endX - startX + 1] : new Geo[0, 0];
                    var oldXMCopy = _verticalMirror && _horizontalMirror && _fourthMirrorCursor ? new Geo[endY - startY + 1, endX - startX + 1] : new Geo[0, 0];
                    ...
                    for (var y = startY; y <= endY; y++)
                    {
                        for (var x = startX; x <= endX; x++)
                        {
                            ...
                            // Mirror axis
                            var vvx = x + (_verticalMirrorPos / 20 - x) * 2;
                            var vvy = y + (_horizontalMirrorPos / 20 - y) * 2;

                            if (GLOBALS.Level.Width % 2 == 0) vvx--;
                            if (GLOBALS.Level.Height % 2 == 0) vvy--;
                            ...
                            if (_eraseAllMode)
                            {
                                ...
                                if (_verticalMirror)
                                {
                                    if (vvx >= 0 && vvx < GLOBALS.Level.Width)
                                    {
                                        var vcell = GLOBALS.Level.GeoMatrix[y, vvx, GLOBALS.Layer];
                                        var voldCell = vcell;

                                        vcell.Type = GeoType.Air;
                                        vcell.Features = GeoFeature.None;

                                        oldVMCopy[y - startY, x - startX] = voldCell;
                                        newVMCopy[y - startY, x - startX] = vcell;

                                        GLOBALS.Level.GeoMatrix[y, vvx, GLOBALS.Layer] = vcell;
                                    }
                                }

                                if (_horizontalMirror)
                                {
                                    if (vvy >= 0 && vvy < GLOBALS.Level.Height)
                                    {
                                        var vcell = GLOBALS.Level.GeoMatrix[vvy, x, GLOBALS.Layer];
                                        var voldCell = vcell;

                                        vcell.Type = GeoType.Air;
                                        vcell.Features = GeoFeature.None;

                                        oldHMCopy[y - startY, x - startX] = voldCell;
                                        newHMCopy[y - startY, x - startX] = vcell;

                                        GLOBALS.Level.GeoMatrix[vvy, x, GLOBALS.Layer] = vcell;
                                    }
                                }

                                if (_verticalMirror && _horizontalMirror && _fourthMirrorCursor)
                                {
                                    if (vvx >= 0 && vvx < GLOBALS.Level.Width && vvy >= 0 && vvy < GLOBALS.Level.Height)
                                    {
                                        var xcell = GLOBALS.Level.GeoMatrix[vvy, vvx, GLOBALS.Layer];
                                        var xoldCell = xcell;

                                        xcell.Type = GeoType.Air;
                                        xcell.Features = GeoFeature.None;

                                        oldXMCopy[y - startY, x - startX] = xoldCell;
                                        newXMCopy[y - startY, x - startX] = xcell;

                                        GLOBALS.Level.GeoMatrix[vvy, vvx, GLOBALS.Layer] = xcell;
                                    }
                                }
                            ...
                            else
                            {
                                ...
                                var vcell = _verticalMirror && vvx >= 0 && vvx < GLOBALS.Level.Width ? GLOBALS.Level.GeoMatrix[y, vvx, GLOBALS.Layer] : GLOBALS.Level.GeoMatrix[y, x, GLOBALS.Layer];
                                var hcell = _horizontalMirror && vvy >= 0 && vvy < GLOBALS.Level.Height ? GLOBALS.Level.GeoMatrix[vvy, x, GLOBALS.Layer] : GLOBALS.Level.GeoMatrix[y, x, GLOBALS.Layer];
                                var xcell = _verticalMirror && _horizontalMirror && vvx < GLOBALS.Level.Width && vvy >= 0 && vvy < GLOBALS.Level.Height ? GLOBALS.Level.GeoMatrix[vvy, vvx, GLOBALS.Layer] : GLOBALS.Level.GeoMatrix[y, x, GLOBALS.Layer];

                                oldCopy[y - startY, x - startX] = cell;
                                if (_verticalMirror) oldVMCopy[y - startY, x - startX] = vcell;
                                if (_horizontalMirror) oldHMCopy[y - startY, x - startX] = hcell;
                                if (_verticalMirror && _horizontalMirror && _fourthMirrorCursor) oldHMCopy[y - startY, x - startX] = xcell;
                                    ...
                                    case 0:
                                        {
                                            ...
                                            if (id == GeoType.SlopeNE)
                                            {
                                                ...
                                                if (slope != -1)
                                                {
                                                    ...
                                                    vcell.Type = (GeoType)slope;
                                                    hcell.Type = (GeoType)slope;
                                                    xcell.Type = (GeoType)slope;
                                                }
                                            }
                                            else
                                            {
                                    ...
                                    case 1:
                                        {
                                            ...
                                            vcell.ToggleWhen(id, !_eraseMode);
                                            hcell.ToggleWhen(id, !_eraseMode);
                                            xcell.ToggleWhen(id, !_eraseMode);
                                        }
                                        break;
                                    ...
                                    case 2:
                                        {
                                            ...
                                            if (_verticalMirror && GLOBALS.Settings.GeometryEditor.AllowOutboundsPlacement ||
                                                (vvx * scale >= GLOBALS.Level.Border.X ||
                                                vvx * scale < GLOBALS.Level.Border.Width + GLOBALS.Level.Border.X))
                                            {
                                                vcell.Enable(id);
                                            }

                                            if (_horizontalMirror && GLOBALS.Settings.GeometryEditor.AllowOutboundsPlacement ||
                                                (vvy * scale >= GLOBALS.Level.Border.Y ||
                                                vvy * scale < GLOBALS.Level.Border.Height + GLOBALS.Level.Border.Y))
                                            {
                                                hcell.Enable(id);
                                            }

                                            if (_verticalMirror && _horizontalMirror && _fourthMirrorCursor && GLOBALS.Settings.GeometryEditor.AllowOutboundsPlacement ||
                                                (vvy * scale >= GLOBALS.Level.Border.Y ||
                                                vvy * scale < GLOBALS.Level.Border.Height + GLOBALS.Level.Border.Y || vvx * scale >= GLOBALS.Level.Border.X ||
                                                vvx * scale < GLOBALS.Level.Border.Width + GLOBALS.Level.Border.X))
                                            {
                                                xcell.Enable(id);
                                            }
                                ...
                                if (_verticalMirror && vvx >= 0 && vvx < GLOBALS.Level.Width)
                                {
                                    GLOBALS.Level.GeoMatrix[y, vvx, GLOBALS.Layer] = vcell;
                                }
                                if (_horizontalMirror && vvy >= 0 && vvy < GLOBALS.Level.Height)
                                {
                                    GLOBALS.Level.GeoMatrix[vvy, x, GLOBALS.Layer] = hcell;
                                }
                                if (_verticalMirror && _horizontalMirror && _fourthMirrorCursor &&  vvx >= 0 && vvx < GLOBALS.Level.Width && vvy >= 0 && vvy < GLOBALS.Level.Height)
                                {
                                    GLOBALS.Level.GeoMatrix[vvy, vvx, GLOBALS.Layer] = xcell;
                                }
                    ...
                    var vx = startX + endX + (_verticalMirrorPos / 20 - (startX + endX)) * 2;
                    var vy = startY + endY + (_horizontalMirrorPos / 20 - (startY + endY)) * 2;

                    if (GLOBALS.Level.Width % 2 == 0) vx--;
                    if (GLOBALS.Level.Height % 2 == 0) vy--;

                    if (vx >= 0 && vx < GLOBALS.Level.Width) _gram.Proceed((vx, startY, GLOBALS.Layer), oldVMCopy, newVMCopy);
                    if (vy >= 0 && vy < GLOBALS.Level.Height) _gram.Proceed((startX, vy, GLOBALS.Layer), oldHMCopy, newHMCopy);
                }
                    ...
                    for (var x = startX; x <= endX; x++)
                    {
                        ...
                        if (_eraseAllMode)
                        {
                            ...
                            if (_horizontalMirror)
                            {
                                if (vvy >= 0 && vvy < GLOBALS.Level.Height)
                                {
                                    var hcell = GLOBALS.Level.GeoMatrix[vvy, x, GLOBALS.Layer];
                                    var holdCell = hcell;

                                    hcell.Type = GeoType.Air;
                                    hcell.Features = GeoFeature.None;

                                    oldHMCopy[y - startY, x - startX] = holdCell;
                                    newHMCopy[y - startY, x - startX] = hcell;
                        ...
                        else
                        {
                            ...
                            var vcell = _verticalMirror && vvx >= 0 && vvx < GLOBALS.Level.Width ? GLOBALS.Level.GeoMatrix[y, vvx, GLOBALS.Layer] : GLOBALS.Level.GeoMatrix[y, x, GLOBALS.Layer];
                            var hcell = _horizontalMirror && vvy >= 0 && vvy < GLOBALS.Level.Height ? GLOBALS.Level.GeoMatrix[vvy, x, GLOBALS.Layer] : GLOBALS.Level.GeoMatrix[y, x, GLOBALS.Layer];
                            var xcell = _verticalMirror && _horizontalMirror && _fourthMirrorCursor && vvx >= 0 && vvx < GLOBALS.Level.Width && vvy >= 0 && vvy < GLOBALS.Level.Height ? GLOBALS.Level.GeoMatrix[vvy, vvx, GLOBALS.Layer] : GLOBALS.Level.GeoMatrix[y, x, GLOBALS.Layer];
        ...
        else
        {
            if ((_shortcuts.Erase.Check(ctrl, shift, alt, true) || _shortcuts.AltErase.Check(ctrl, shift, alt, true)) && canDrawGeo && inMatrixBounds)
            {
                ...
                var vx = matrixX + (_verticalMirrorPos / 20 - matrixX) * 2;
                var vy = matrixY + (_horizontalMirrorPos / 20 - matrixY) * 2;
                    ...
                    case 0:
                        {
                            ...
                            if (id is GeoType.SlopeNE)
                            {
                                ...
                                var vcell = _verticalMirror && vx >= 0 && vx < GLOBALS.Level.Width ? GLOBALS.Level.GeoMatrix[matrixY, vx, GLOBALS.Layer] : GLOBALS.Level.GeoMatrix[matrixY, matrixX, GLOBALS.Layer];
                                var hcell = _horizontalMirror && vy >= 0 && vy < GLOBALS.Level.Height ? GLOBALS.Level.GeoMatrix[vy, matrixX, GLOBALS.Layer] : GLOBALS.Level.GeoMatrix[matrixY, matrixX, GLOBALS.Layer];
                                var xcell = _fourthMirrorCursor && _verticalMirror && _horizontalMirror && vx >= 0 && vx < GLOBALS.Level.Width && vy >= 0 && vy < GLOBALS.Level.Height ? GLOBALS.Level.GeoMatrix[vy, vx, GLOBALS.Layer] : GLOBALS.Level.GeoMatrix[matrixY, matrixX, GLOBALS.Layer];
                                ...
                                if (matrixX != _prevCoordsX || matrixY != _prevCoordsY || !_clickTracker)
                                {
                                    ...
                                    var newVCell = new Geo { Type = GeoType.Air, Features = vcell.Features };
                                    var newHCell = new Geo { Type = GeoType.Air, Features = hcell.Features };
                                    var newXCell = new Geo { Type = GeoType.Air, Features = xcell.Features };
                                    ...
                                    var vaction = new GeoGram.CellAction(new Coords(vx, matrixY, GLOBALS.Layer), oldVCell, newVCell);
                                    var haction = new GeoGram.CellAction(new Coords(matrixX, vy, GLOBALS.Layer), oldHCell, newHCell);
                                    var xaction = new GeoGram.CellAction(new Coords(vx, vy, GLOBALS.Layer), oldXCell, newXCell);

                                    _groupedActions.Add(action);
                                    _groupedActions.Add(vaction);
                                    _groupedActions.Add(haction);
                                    _groupedActions.Add(xaction);
                            ...
                            else
                            {
                                if (_brushRadius > 0)
                                {
                                    ...
                                    if (_prevCoordsX != matrixX || _prevCoordsY != matrixY || !_clickTracker)
                                    {
                                        ...
                                        if (_circularBrush)
                                        {
                                            ...
                                            if (_verticalMirror)
                                            {
                                                actions = [.. actions, .. PlaceGeoCircularBrush(vx, matrixY, _brushRadius, 0)];
                                            }

                                            if (_horizontalMirror)
                                            {
                                                actions = [.. actions, .. PlaceGeoCircularBrush(matrixX, vy, _brushRadius, 0)];
                                            }

                                            if (_verticalMirror && _horizontalMirror && _fourthMirrorCursor)
                                            {
                                                actions = [.. actions, .. PlaceGeoCircularBrush(vx, vy, _brushRadius, 0)];
                                            }
                                        }
                                        else
                                        {
                                            ...
                                            if (_verticalMirror)
                                            {
                                                actions = [.. actions, .. PlaceGeoSquareBrush(vx, matrixY, _brushRadius, 0)];
                                            }

                                            if (_horizontalMirror)
                                            {
                                                actions = [.. actions, .. PlaceGeoSquareBrush(matrixX, vy, _brushRadius, 0)];
                                            }

                                            if (_verticalMirror && _horizontalMirror && _fourthMirrorCursor)
                                            {
                                                actions = [.. actions, .. PlaceGeoSquareBrush(vx, vy, _brushRadius, 0)];
                                            }
                                        }
                                ...
                                else
                                {
                                    ...
                                    var vcell = _verticalMirror && vx >= 0 && vx < GLOBALS.Level.Width ? GLOBALS.Level.GeoMatrix[matrixY, vx, GLOBALS.Layer] : GLOBALS.Level.GeoMatrix[matrixY, matrixX, GLOBALS.Layer];
                                    var hcell = _horizontalMirror && vy >= 0 && vy < GLOBALS.Level.Height ? GLOBALS.Level.GeoMatrix[vy, matrixX, GLOBALS.Layer] : GLOBALS.Level.GeoMatrix[matrixY, matrixX, GLOBALS.Layer];
                                    var xcell = _horizontalMirror && _verticalMirror && _fourthMirrorCursor && vx >= 0 && vx < GLOBALS.Level.Width && vy >= 0 && vy < GLOBALS.Level.Height ? GLOBALS.Level.GeoMatrix[vy, vx, GLOBALS.Layer] : GLOBALS.Level.GeoMatrix[matrixY, matrixX, GLOBALS.Layer];
                                    ...
                                    if (_prevCoordsX != matrixX || _prevCoordsY != matrixY || !_clickTracker)
                                    {
                                        ...
                                        if (_verticalMirror && vx >= 0 && vx < GLOBALS.Level.Width && vcell.Type is not GeoType.Air)
                                        {
                                            var oldVCell = vcell;
                                            var newVCell = vcell with { Type = GeoType.Air };

                                            GLOBALS.Level.GeoMatrix[matrixY, vx, GLOBALS.Layer] = newVCell;

                                            var action = new GeoGram.CellAction(new Coords(vx, matrixY, GLOBALS.Layer), oldVCell, newVCell);
                                        ...
                                        if (_horizontalMirror && vy >= 0 && vy < GLOBALS.Level.Height && hcell.Type is not GeoType.Air)
                                        {
                                            var oldHCell = hcell;
                                            var newHCell = hcell with { Type = GeoType.Air };

                                            GLOBALS.Level.GeoMatrix[vy, matrixX, GLOBALS.Layer] = newHCell;

                                            var action = new GeoGram.CellAction(new Coords(matrixX, vy, GLOBALS.Layer), oldHCell, newHCell);
                                        ...
                                        if (_fourthMirrorCursor && _verticalMirror && _horizontalMirror && vx >= 0 && vx < GLOBALS.Level.Width && vy >= 0 && vy < GLOBALS.Level.Height && hcell.Type is not GeoType.Air)
                                        {
                                            var oldXCell = xcell;
                                            var newXCell = xcell with { Type = GeoType.Air };

                                            GLOBALS.Level.GeoMatrix[vy, vx, GLOBALS.Layer] = newXCell;

                                            var action = new GeoGram.CellAction(new Coords(vx, vy, GLOBALS.Layer), oldXCell, newXCell);
                    ...
                    case 1:
                        {
                            ...
                            if (_prevCoordsX != matrixX || _prevCoordsY != matrixY || !_clickTracker)
                            {
                                if (cell[id])
                                {
                                    ...
                                    var action = new GeoGram.CellAction(new Coords(matrixX, matrixY, GLOBALS.Layer), oldCell, cell);
                                ...
                                if (_verticalMirror && vx >= 0 && vx < GLOBALS.Level.Width && vcell[id])
                                {
                                    var oldVCell = vcell;

                                    vcell.Disable(id);

                                    GLOBALS.Level.GeoMatrix[matrixY, vx, GLOBALS.Layer] = vcell;

                                    var action = new GeoGram.CellAction(new Coords(vx, matrixY, GLOBALS.Layer), oldVCell, vcell);
                                ...
                                if (_horizontalMirror && vy >= 0 && vy < GLOBALS.Level.Height && hcell[id])
                                {
                                    var oldHCell = hcell;

                                    hcell.Disable(id);

                                    GLOBALS.Level.GeoMatrix[vy, matrixX, GLOBALS.Layer] = hcell;

                                    var action = new GeoGram.CellAction(new Coords(matrixX, vy, GLOBALS.Layer), oldHCell, hcell);
                                ...
                                if (_fourthMirrorCursor && _verticalMirror && _horizontalMirror && vy >= 0 && vx >= 0 && vx < GLOBALS.Level.Width && vy < GLOBALS.Level.Height && hcell[id])
                                {
                                    var oldXCell = xcell;

                                    xcell.Disable(id);

                                    GLOBALS.Level.GeoMatrix[vy, vx, GLOBALS.Layer] = xcell;

                                    var action = new GeoGram.CellAction(new Coords(vx, vy, GLOBALS.Layer), oldXCell, xcell);
                    ...
                    case 2:
                        {
                            ...
                            if (!(_prevCoordsX != matrixX || _prevCoordsY != matrixY || !_clickTracker)) break;
                    ...
                    case 0:
                        {
                            ...
                            if (id is GeoType.SlopeNE)
                            {
                                ...
                                var vcell = _verticalMirror && vx >= 0 && vx < GLOBALS.Level.Width ? GLOBALS.Level.GeoMatrix[matrixY, vx, GLOBALS.Layer] : GLOBALS.Level.GeoMatrix[matrixY, matrixX, GLOBALS.Layer];
                                var hcell = _horizontalMirror && vy >= 0 && vy < GLOBALS.Level.Height ? GLOBALS.Level.GeoMatrix[vy, matrixX, GLOBALS.Layer] : GLOBALS.Level.GeoMatrix[matrixY, matrixX, GLOBALS.Layer];
                                var xcell = _fourthMirrorCursor && _horizontalMirror && _verticalMirror && _fourthMirrorCursor && vx >= 0 && vx < GLOBALS.Level.Width && vy >= 0 && vy < GLOBALS.Level.Height ? GLOBALS.Level.GeoMatrix[vy, vx, GLOBALS.Layer] : GLOBALS.Level.GeoMatrix[matrixY, matrixX, GLOBALS.Layer];
                                ...
                                if (matrixX != _prevCoordsX || matrixY != _prevCoordsY || !_clickTracker)
                                {
                                    ...
                                    if (_verticalMirror && vx >= 0 && vx < GLOBALS.Level.Width)
                                    {
                                        var oldVCell = vcell;
                                        var newVCell = vcell with { Type = id };
                                    ...
                                    if (_horizontalMirror && vy >= 0 && vy < GLOBALS.Level.Height)
                                    {
                                        var oldHCell = hcell;
                                        var newHCell = hcell with { Type = id };
                                    ...
                                    if (_fourthMirrorCursor && _verticalMirror && _horizontalMirror && vx >= 0 && vx < GLOBALS.Level.Width && vy >= 0 && vy < GLOBALS.Level.Height)
                                    {
                                        var oldXCell = xcell;
                                        var newXCell = xcell with { Type = id };

                                        xcell.Type = (GeoType)slope;

                                        var xaction = new GeoGram.CellAction(new Coords(matrixX, vy, GLOBALS.Layer), oldXCell, newXCell);
                            ...
                            else
                            {
                                if (_brushRadius > 0)
                                {
                                    ...
                                    if (_prevCoordsX != matrixX || _prevCoordsY != matrixY || !_clickTracker)
                                    {
                                        ...
                                        if (_circularBrush)
                                        {
                                            ...
                                            if (_verticalMirror && vx >= 0 && vx < GLOBALS.Level.Width)
                                            {
                                                actions = [.. actions, .. PlaceGeoCircularBrush(vx, matrixY, _brushRadius, id)];
                                            }
                                            if (_horizontalMirror && vy >= 0 && vy < GLOBALS.Level.Height)
                                            {
                                                actions = [.. actions, .. PlaceGeoCircularBrush(matrixX, vy, _brushRadius, id)];
                                            }
                                            if (_fourthMirrorCursor && _verticalMirror && _horizontalMirror && vx >= 0 && vx < GLOBALS.Level.Width && vy >= 0 && vy < GLOBALS.Level.Height)
                                            {
                                                actions = [.. actions, .. PlaceGeoCircularBrush(vx, vy, _brushRadius, id)];
                                            }
                                        }
                                        else
                                        {
                                            ...
                                            if (_verticalMirror && vx >= 0 && vx < GLOBALS.Level.Width)
                                            {
                                                actions = [.. actions, .. PlaceGeoSquareBrush(vx, matrixY, _brushRadius, id)];
                                            }
                                            if (_horizontalMirror && vy >= 0 && vy < GLOBALS.Level.Height)
                                            {
                                                actions = [.. actions, .. PlaceGeoSquareBrush(matrixX, vy, _brushRadius, id)];
                                            }
                                            if (_fourthMirrorCursor && _verticalMirror && _horizontalMirror && vx >= 0 && vx < GLOBALS.Level.Width && vy >= 0 && vy < GLOBALS.Level.Height)
                                            {
                                                actions = [.. actions, .. PlaceGeoSquareBrush(vx, vy, _brushRadius, id)];
                                            }
                                        }
                                ...
                                else
                                {
                                    ...
                                    if (_prevCoordsX != matrixX || _prevCoordsY != matrixY || !_clickTracker)
                                    {
                                        ...
                                        if (_verticalMirror && vx >= 0 && vx < GLOBALS.Level.Width && vcell.Type != id)
                                        {
                                            var oldCell = vcell;
                                            var newCell = vcell with { Type = id };

                                            GLOBALS.Level.GeoMatrix[matrixY, vx, GLOBALS.Layer] = newCell;

                                            var action = new GeoGram.CellAction(new Coords(vx, matrixY, GLOBALS.Layer), oldCell, newCell);
                                        ...
                                        if (_horizontalMirror && vy >= 0 && vy < GLOBALS.Level.Height && hcell.Type != id)
                                        {
                                            var oldCell = hcell;
                                            var newCell = hcell with { Type = id };

                                            GLOBALS.Level.GeoMatrix[vy, matrixX, GLOBALS.Layer] = newCell;

                                            var action = new GeoGram.CellAction(new Coords(matrixX, vy, GLOBALS.Layer), oldCell, newCell);
                                        ...
                                        if (_verticalMirror && _horizontalMirror && vx >= 0 && vx < GLOBALS.Level.Width && vy >= 0 && vy < GLOBALS.Level.Height && xcell.Type != id)
                                        {
                                            var oldCell = xcell;
                                            var newCell = xcell with { Type = id };

                                            GLOBALS.Level.GeoMatrix[vy, vx, GLOBALS.Layer] = newCell;

                                            var action = new GeoGram.CellAction(new Coords(vx, vy, GLOBALS.Layer), oldCell, newCell);
                    ...
                    case 1:
                        {
                            ...
                            if (_prevCoordsX != matrixX || _prevCoordsY != matrixY || !_clickTracker)
                            {
                                ...
                                if (_verticalMirror && _horizontalMirror && _fourthMirrorCursor && vx >= 0 && vx < GLOBALS.Level.Width && vy >= 0 && vy < GLOBALS.Level.Height && !hcell[id])
                                {
                    ...
                    case 2:
                        {
                            ...
                            if (!cell[id] && !(!GLOBALS.Settings.GeometryEditor.AllowOutboundsPlacement &&
                            ...
                            if (_verticalMirror && vx >= 0 && vx < GLOBALS.Level.Width && !vcell[id] && !(!GLOBALS.Settings.GeometryEditor.AllowOutboundsPlacement &&
                                (vx * scale < GLOBALS.Level.Border.X ||
                                vx * scale >= GLOBALS.Level.Border.Width + GLOBALS.Level.Border.X ||
                                matrixY * scale < GLOBALS.Level.Border.Y ||
                                matrixY * scale >= GLOBALS.Level.Border.Height + GLOBALS.Level.Border.Y))) {
                                var oldCell = vcell;
                            ...
                            if (_horizontalMirror && vy >= 0 && vy < GLOBALS.Level.Height && !hcell[id] && ! (!GLOBALS.Settings.GeometryEditor.AllowOutboundsPlacement &&
                                (matrixX * scale < GLOBALS.Level.Border.X ||
                                matrixX * scale >= GLOBALS.Level.Border.Width + GLOBALS.Level.Border.X ||
                                vy * scale < GLOBALS.Level.Border.Y ||
                                vy * scale >= GLOBALS.Level.Border.Height + GLOBALS.Level.Border.Y))) {
                                var oldCell = hcell;
                            ...
                            if (_verticalMirror && 
                                ...
                                _fourthMirrorCursor && 
                                vx >= 0 && 
                                vx < GLOBALS.Level.Width && 
                                vy >= 0 && 
                                vy < GLOBALS.Level.Height && 
                                !xcell[id] && !(!GLOBALS.Settings.GeometryEditor.AllowOutboundsPlacement &&
                                (vx * scale < GLOBALS.Level.Border.X ||
                                vx * scale >= GLOBALS.Level.Border.Width + GLOBALS.Level.Border.X ||
                                vy * scale < GLOBALS.Level.Border.Y ||
                                vy * scale >= GLOBALS.Level.Border.Height + GLOBALS.Level.Border.Y))) {
        ...
        // BeginDrawing();
        {
            ...
            BeginMode2D(_camera);
            {
                ...
                if (_verticalMirror)
                {
                    DrawLineEx(new Vector2(_verticalMirrorPos, -10), new Vector2(_verticalMirrorPos, GLOBALS.Level.Height * 20 + 20), 4, Color.White);
                    DrawLineEx(new Vector2(_verticalMirrorPos, -10), new Vector2(_verticalMirrorPos, GLOBALS.Level.Height * 20 + 20), 3, Color.Black);
                    DrawLineEx(new Vector2(_verticalMirrorPos, -10), new Vector2(_verticalMirrorPos, GLOBALS.Level.Height * 20 + 20), 2, Color.Green);

                    if (_movingMirrorLock is 0 or -1 && _mirrorCursor)
                    {
                        var pos = new Vector2(matrixX * 20 + (_verticalMirrorPos - matrixX * 20) * 2 - 20, matrixY * 20);

                        if (_multiselect) {
                            DrawRectangleLinesEx(_selectionRectangle with { X = _selectionRectangle.X + _selectionRectangle.Width + (_verticalMirrorPos - (_selectionRectangle.X + _selectionRectangle.Width)) * 2 }, 2, _eraseMode
                                            ? Color.Red
                                            : (_memLoadMode
                            ...
                            if (_brushRadius > 0)
                            {
                                if (_circularBrush)
                                {
                                    Printers.DrawCircularSquareLines((int)pos.X/20, matrixY, _brushRadius, 20, 2, _eraseMode ? Color.Red : Color.White);
                                }
                                else
                                {
                                    DrawRectangleLinesEx(
                                        new Rectangle((pos.X/20 - _brushRadius) * scale, (matrixY - _brushRadius) * scale, (_brushRadius * 2 + 1) * scale, (_brushRadius * 2 + 1) * scale),
                                        2,
                                        _eraseMode
                                            ? Color.Red
                                            : (_memLoadMode
                            ...
                            {
                                DrawRectangleLinesEx(new Rectangle(pos, 20, 20), 2, _eraseMode
                                            ? Color.Red
                                            : (_memLoadMode
                                                ? new Color(89, 7, 222, 255)
                                                : Color.White
                        ...
                        if (_labeledMirrorCursors)
                        {
                            if (GLOBALS.Font is { } f)
                            {
                                DrawTextEx(f, "2", pos + new Vector2(5, 0), 22, 0, Color.White);
                            }
                            else
                            {
                                DrawTextEx(GetFontDefault(), "2", pos + new Vector2(5, 0), 22, 0, Color.White);
                            }
                        }
                    }
                }
                ...
                if (_horizontalMirror)
                {
                    DrawLineEx(new Vector2(-10, _horizontalMirrorPos), new Vector2(GLOBALS.Level.Width * 20 + 20, _horizontalMirrorPos), 4, Color.White);
                    DrawLineEx(new Vector2(-10, _horizontalMirrorPos), new Vector2(GLOBALS.Level.Width * 20 + 20, _horizontalMirrorPos), 3, Color.Black);
                    DrawLineEx(new Vector2(-10, _horizontalMirrorPos), new Vector2(GLOBALS.Level.Width * 20 + 20, _horizontalMirrorPos), 2, Color.Red);

                    if (_movingMirrorLock is 0 or -1 && _mirrorCursor)
                    {
                        var pos = new Vector2(matrixX * 20, matrixY * 20 + (_horizontalMirrorPos - matrixY * 20) * 2 - 20);

                        if (_multiselect)
                        {
                            DrawRectangleLinesEx(_selectionRectangle with { Y = _selectionRectangle.Y + _selectionRectangle.Height + (_horizontalMirrorPos - (_selectionRectangle.Y + _selectionRectangle.Height)) * 2 }, 2, _eraseMode
                                        ? Color.Red
                                        : (_memLoadMode
                                            ? new Color(89, 7, 222, 255)
                                            : Color.White
                        ...
                        else
                        {
                            if (_brushRadius > 0)
                            {
                                if (_circularBrush)
                                {
                                    Printers.DrawCircularSquareLines(matrixX, (int)pos.Y/20, _brushRadius, 20, 2, _eraseMode ? Color.Red : Color.White);
                                }
                                else
                                {
                                    DrawRectangleLinesEx(
                                        new Rectangle((matrixX - _brushRadius) * scale, (pos.Y/20 - _brushRadius) * scale, (_brushRadius * 2 + 1) * scale, (_brushRadius * 2 + 1) * scale),
                                        2,
                                        _eraseMode
                                            ? Color.Red
                                            : (_memLoadMode
                        ...
                        if (_labeledMirrorCursors)
                        {
                            if (GLOBALS.Font is { } f)
                            {
                                DrawTextEx(f, "3", pos + new Vector2(5, 0), 22, 0, Color.White);
                            }
                            else
                            {
                                DrawTextEx(GetFontDefault(), "3", pos + new Vector2(5, 0), 22, 0, Color.White);
                            }
                        }
                    }
                }

                if (_movingMirrorLock is 0 or -1 && _mirrorCursor && _fourthMirrorCursor && _horizontalMirror && _verticalMirror)
                {
                    var pos = new Vector2(
                        _selectionRectangle.X + _selectionRectangle.Width + (_verticalMirrorPos - (_selectionRectangle.X + _selectionRectangle.Width)) * 2,
                        _selectionRectangle.Y + _selectionRectangle.Height + (_horizontalMirrorPos - (_selectionRectangle.Y + _selectionRectangle.Height)) * 2);

                    if (_multiselect)
                    {
                        DrawRectangleLinesEx(_selectionRectangle with { X = pos.X, Y = pos.Y }, 2, _eraseMode
                                        ? Color.Red
                                        : (_memLoadMode
                                            ? new Color(89, 7, 222, 255)
                                            : Color.White
                    ...
                    else
                    {
                        if (_brushRadius > 0)
                        {
                            if (_circularBrush)
                            {
                                Printers.DrawCircularSquareLines(matrixX + (_verticalMirrorPos/20 - matrixX)*2 - 1, matrixY + (_horizontalMirrorPos/20 - matrixY) * 2 - 1, _brushRadius, 20, 2, _eraseMode ? Color.Red : Color.White);
                            }
                            else
                            {
                                DrawRectangleLinesEx(
                                    new Rectangle((matrixX + (_verticalMirrorPos/20 - matrixX)*2 - 1 - _brushRadius) * scale, (matrixY + (_horizontalMirrorPos/20 - matrixY) * 2 - 1 - _brushRadius) * scale, (_brushRadius * 2 + 1) * scale, (_brushRadius * 2 + 1) * scale),
                                    2,
                                    _eraseMode
                                        ? Color.Red
                                        : (_memLoadMode
                        ...
                        {
                            DrawRectangleLinesEx(new Rectangle(new Vector2(matrixX * 20 + (_verticalMirrorPos - matrixX * 20)*2 - 20, matrixY * 20 + (_horizontalMirrorPos - matrixY * 20) * 2 - 20), 20, 20), 2,  _eraseMode
                                    ? Color.Red
                                    : (_memLoadMode
                                        ? new Color(89, 7, 222, 255)
                                        : Color.White
                    ...
                    if (_labeledMirrorCursors)
                    {
                        if (GLOBALS.Font is { } f)
                        {
                            DrawTextEx(f, "4", new Vector2(matrixX * 20 + (_verticalMirrorPos - matrixX * 20) * 2, matrixY * 20 + (_horizontalMirrorPos - matrixY * 20) * 2 - 20) + new Vector2(5, 0), 22, 0, Color.White);
                        }
                        else
                        {
                            DrawTextEx(GetFontDefault(), "4", pos + new Vector2(5, 0), 22, 0, Color.White);
                        }
                    }
                }
            ...
            var mirrorOpened = ImGui.Begin("Mirror Tools##GeometryEditorMirrorWindow");

            var mirrorPos = ImGui.GetWindowPos();
            var mirrorWinSpace = ImGui.GetWindowSize();

            _isMirrorWinHovered = CheckCollisionPointRec(GetMousePosition(), new(mirrorPos.X - 5, mirrorPos.Y - 5, mirrorWinSpace.X + 10, mirrorWinSpace.Y + 10));

            if (mirrorOpened)
            {
                ImGui.Checkbox("Vertical", ref _verticalMirror);
                ImGui.Checkbox("Horizontal", ref _horizontalMirror);

                ImGui.Checkbox("Cursor", ref _mirrorCursor);

                if (!_mirrorCursor) ImGui.BeginDisabled();
                ImGui.Checkbox("Fourth Cursor", ref _fourthMirrorCursor);
                ImGui.Checkbox("Label Cursors", ref _labeledMirrorCursors);
                if (!_mirrorCursor) ImGui.EndDisabled();
            ...
            if (GLOBALS.Settings.GeneralSettings.ShortcutWindow)
            {
                ...
                _isShortcutsWinHovered = CheckCollisionPointRec(
                    ...
                    {
                        X = shortcutWindowRect.X - 5,
                        Width = shortcutWindowRect.Width + 10
                    }
                );
