...
internal class PropsEditorPage : EditorPage, IContextListener
{
    ...
    private RenderTexture2D _presetPreviewRT = new(0, 0);
    ...
    private void UpdatePresetsPreview(int index)
    {
        ...
        if (_presetPreviewRT.Raw.Texture.Width != (int) rect.Width && _presetPreviewRT.Raw.Texture.Height != (int) rect.Height)
        {
            _presetPreviewRT.Dispose();
            _presetPreviewRT = new((int)rect.Width, (int)rect.Height);
        }

        BeginTextureMode(_presetPreviewRT);
        ClearBackground(GLOBALS.Settings.GeneralSettings.DarkTheme ? Color.Black with { A = 0 } : Color.White);

        foreach (var p in preset)
        {
            var (category, i) = p.Position;
            var quads = p.Quad;
    ...
    private int MenuRootCategoryIndex { 
        get => _menuRootCategoryIndex; 
        set 
        {
            _menuRootCategoryIndex = value;
    ...
    private bool _isPresetsWinHovered;
    ...
    public override void Draw()
    {
        ...
        switch (_mode)
        {
            case 1: // Place Mode
                ...
                if (_selectedPreset != -1 && _presets.SavedProps.Count != 0)
                {
                    if (_shortcuts.PlaceProp.Check(ctrl, shift, alt))
                    {
                        var rect = Utils.EncloseProps(_presets.SavedProps[_selectedPreset].Props.Select(p => p.Quad));
                        var center = new Vector2((rect.X + rect.Width)/2f, (rect.Y + rect.Height)/2f);

                        var propsToPlace = _presets.SavedProps[_selectedPreset].Props
                            .Select(p => {
                                var clone = p.Clone();
                                var newQuad = clone.Quad + GetScreenToWorld2D(GetMousePosition(), _camera) - center;
                                clone.Quad = newQuad;
                                return clone;
                            });

                        GLOBALS.Level.Props = [..GLOBALS.Level.Props, ..propsToPlace];
                        _selected = new bool[GLOBALS.Level.Props.Length];
                        _hidden = new bool[GLOBALS.Level.Props.Length];
                ...
                {
                    MenuRootCategoryIndex++;
                    if (_menuRootCategoryIndex > 3) MenuRootCategoryIndex = 0;
                ...
                {
                    MenuRootCategoryIndex--;
                    if (_menuRootCategoryIndex < 0) MenuRootCategoryIndex = 3;
            ...
            case 0: // Select Mode
                ...
                else
                {
                    if (anySelected)
                    {
                        ...
                        if (_shortcuts.AddPreset.Check(ctrl, shift, alt))
                        {
                            _presets.Add(fetchedSelected.Select(f => f.prop));
                        }
                    }
            ...
            switch (_mode)
            {
                case 1: // Place Mode
                    if (_selectedPreset != -1 && _presets.SavedProps.Count != 0)
                    {
                        var rect = Utils.EncloseProps(_presets.SavedProps[_selectedPreset].Props.Select(p => p.Quad));
                        var center = new Vector2((rect.X + rect.Width)/2f, (rect.Y + rect.Height)/2f);
                        var pos = GetScreenToWorld2D(GetMousePosition(), _camera);

                        var fetchedProps = _presets.SavedProps[_selectedPreset].Props.Select(p => {
                            var clone = p.Clone();
                            var newQuad = clone.Quad + (pos - center);
                            clone.Quad = newQuad;
                            return clone;
                        });

                        foreach (var p in fetchedProps)
                        {
                            var (category, index) = p.Position;
                            var quads = p.Quad;
...

        {
            ...
            if (menuOpened)
            {
                ...
                
                {
                    ...
                    if (tilesSelected) MenuRootCategoryIndex = 0;
                    if (ropesSelected) MenuRootCategoryIndex = 1;
                    if (longsSelected) MenuRootCategoryIndex = 2;
                    if (othersSelected) MenuRootCategoryIndex = 3;
            ...
            if (GLOBALS.Settings.PropEditor.PresetsWindow) presetsOpened = ImGui.Begin("Props Presets##PropsPresetsWindow");

            var presetsPos = ImGui.GetWindowPos();
            var presetsSize = ImGui.GetWindowSize();

            _isPresetsWinHovered = CheckCollisionPointRec(tileMouse, new(presetsPos.X - 5, presetsPos.Y, presetsSize.X + 10, presetsSize.Y));
            ...
            if (presetsOpened)
            {
                if (ImGui.BeginListBox("##Presets", ImGui.GetContentRegionAvail()))
                {
                    for (var i = 0; i < _presets.SavedProps.Count; i++)
                    {
                        ...
                        if (ImGui.IsItemHovered())
                        {
                            UpdatePresetsPreview(i);

                            if (ImGui.BeginTooltip())
                            {
                                rlImGui.ImageRenderTexture(_presetPreviewRT);
