...
void main() {
    ...
    float totalWidth = fragTexCoord.x * layerWidth + offset.x;

    for (int l = layerNum - 1; l > -1; l--) {
        float currentHeight = fragTexCoord.y * layerHeight + offset.y + l*(layerHeight + offset.y*2);

//        if (l > 0) currentHeight += offset.y;
